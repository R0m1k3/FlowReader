...
export function ArticleCard({ article, onClick, onToggleRead, onToggleFavorite }: ArticleCardProps) {
    ...
    return (
        <article className="magazine-card group cursor-pointer flex flex-col h-full shadow-2xl shadow-carbon/10 hover:shadow-gold/20 transition-all duration-500 rounded-xl border border-transparent hover:border-gold/10" onClick={() => onClick(article)}>
